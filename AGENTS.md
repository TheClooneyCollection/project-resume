...
- `npm run build` — Generates the static site into `_site/` and runs `scripts/generate-pdf.js`, which spins up a local server on an available port to capture `_site/resume.pdf`. The port is assigned at runtime.
